...
step1()
{
	...
	STEP_NAME="REFERENCE MACPORTS WXPYTHON"
	STEP_NUMBER=1
	print_step_starting_message
    port list installed and *wxpython*
    which wx-config
...
step2()
{
    ...
    cd $SOURCE_DIRECTORY/$KICAD_DIRECTORY 
    patch -p0 -N < $PATCH_DIRECTORY/kicad_CMakeLists_boost.patch
    cd $SCRIPT_DIRECTORY
...
step3()
{
	...
	STEP_NAME="REFERENCE MACPORTS WXWIDGETS"
	STEP_NUMBER=3
	print_step_starting_message
    port list installed and *wxWidgets*
    port list installed and *wxgtk*
...
step4()
{
	...
	STEP_NAME="REFERENCE WXPYTHON PYTHON EXTENSIONS"
	STEP_NUMBER=4
	print_step_starting_message
    port list installed and *wxpython*
...
step5()
{
	...
	export wxWidgets_ROOT_DIR=/opt/local
